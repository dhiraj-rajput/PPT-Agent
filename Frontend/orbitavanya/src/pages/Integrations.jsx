      ...
      <div className="mt-auto space-y-2">
        ...
        {status.connected && (
          <button
            ...
            disabled={loading}
            className="w-full rounded-lg py-2 text-xs font-semibold bg-rose-50 text-rose-600 hover:bg-rose-100 border border-transparent disabled:opacity-60 transition-all"
          >
            Disconnect
          </button>
        )}
...
// ---------------------------------------------------------------------------
// LinkedIn, Companies House, SMTP (Using Modal)
// ---------------------------------------------------------------------------
...
function LinkedInCard({ notify, onEdit }) {
  ...
  useEffect(() => {
    if (api.getIntegrationStatus) {
      api.getIntegrationStatus('linkedin')
        .then(res => {
           setStatus(res);
  ...
  return (
    <Card className="flex flex-col">
      ...
      <p className="mt-3 text-sm font-bold text-navy-900 dark:text-white">LinkedIn</p>
      <p className="mt-1 text-xs text-slate-400 dark:text-slate-500 mb-2 flex-1">
      ...
      <button onClick={() => onEdit({
          name: 'linkedin',
          label: 'LinkedIn',
          fields: [{ key: 'li_at', label: 'Cookie token (li_at string)', type: 'password' }]
...
function CompaniesHouseCard({ onEdit }) {
  return (
    <Card className="flex flex-col">
      ...
      <p className="mt-3 text-sm font-bold text-navy-900 dark:text-white">Companies House UK</p>
      <p className="mt-1 text-xs text-slate-400 dark:text-slate-500 mb-2 flex-1">
...
function SmtpCard({ onEdit }) {
  return (
    <Card className="flex flex-col">
      ...
      <p className="mt-3 text-sm font-bold text-navy-900 dark:text-white">Email SMTP</p>
      <p className="mt-1 text-xs text-slate-400 dark:text-slate-500 mb-2 flex-1">
      ...
      <button onClick={() => onEdit({
          ...
          label: 'Email SMTP',
          fields: [
            { key: 'host', label: 'Base URL / Host', type: 'text' },
