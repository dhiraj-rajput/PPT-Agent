...
from api.utils.mailer import send_otp_email, send_invite_email
...
async def _send_otp_for_user(user_id: str, email: str, purpose: str) -> None:
    ...
    try:
        ...
        import logging
        logging.getLogger(__name__).warning(f"[Auth] OTP email failed: {exc}")
...
async def verify_otp(body: VerifyOtpBody):
    ...
    if otp_doc.get("otpHash") != _hash_otp(body.otp.strip()):
