"""
api/routes/users.py
--------------------
User management endpoints for OrbitAvanya — mirrors Node.js server/routes/users.js.

Endpoints:
  GET  /api/users           — list all users (requires auth)
  POST /api/users/invite    — invite a new team member (sends email)
  PATCH /api/users/:id/role — change a user's role
"""

from __future__ import annotations

import os
import re
import secrets
from datetime import datetime, timezone

from bson import ObjectId
from fastapi import APIRouter, Depends, HTTPException
from passlib.context import CryptContext
from pydantic import BaseModel
from typing import Optional

from api.utils.auth import get_current_user
from api.utils.mailer import send_invite_email
from utils.db_client import get_collection

router = APIRouter(prefix="/users", tags=["users"])
_pwd_ctx = CryptContext(schemes=["bcrypt"], deprecated="auto")


def _to_public_user(u: dict) -> dict:
    return {
        "id": str(u["_id"]),
        "name": u.get("name", ""),
        "email": u.get("email", ""),
        "role": u.get("role", "Team Member"),
        "status": "Active" if u.get("isVerified") else "Pending",
        "seed": u.get("email", ""),
        "createdAt": u.get("createdAt", ""),
    }


def _is_valid_email(email: str) -> bool:
    return bool(re.match(r"^[^\s@]+@[^\s@]+\.[^\s@]+$", email))


# ---------------------------------------------------------------------------
# Routes
# ---------------------------------------------------------------------------

@router.get("")
async def list_users(current_user: dict = Depends(get_current_user)):
    users_col = get_collection("users")
    users = list(users_col.find().sort("createdAt", -1))
    return {"users": [_to_public_user(u) for u in users]}


class InviteBody(BaseModel):
    name: Optional[str] = None
    email: str
    role: Optional[str] = "Team Member"


@router.post("/invite", status_code=201)
async def invite_user(
    body: InviteBody,
    current_user: dict = Depends(get_current_user),
):
    if not body.email or not _is_valid_email(body.email):
        raise HTTPException(400, "A valid email address is required.")

    users_col = get_collection("users")
    normalized = body.email.lower().strip()
    existing = users_col.find_one({"email": normalized})
    if existing:
        raise HTTPException(409, "A user with that email already exists.")

    temp_password = secrets.token_urlsafe(9)
    pw_hash = _pwd_ctx.hash(temp_password)
    invitee_name = (body.name or "").strip() or normalized.split("@")[0]

    result = users_col.insert_one({
        "name": invitee_name,
        "email": normalized,
        "phone": "",
        "passwordHash": pw_hash,
        "isVerified": True,
        "role": (body.role or "Team Member").strip(),
        "mustChangePassword": True,
        "invitedBy": current_user["_id"],
        "createdAt": datetime.now(tz=timezone.utc),
    })

    user = users_col.find_one({"_id": result.inserted_id})
    warning = None
    try:
        await send_invite_email(
            to_email=normalized,
            invitee_name=invitee_name,
            role=body.role or "Team Member",
            inviter_name=current_user.get("name"),
            temp_password=temp_password,
        )
    except Exception as exc:
        warning = f"User created, but the invite email could not be sent. Check SMTP settings."

    response = {"user": _to_public_user(user)}
    if warning:
        response["warning"] = warning
    return response


class UpdateRoleBody(BaseModel):
    role: str


@router.patch("/{user_id}/role")
async def update_role(
    user_id: str,
    body: UpdateRoleBody,
    current_user: dict = Depends(get_current_user),
):
    if not body.role:
        raise HTTPException(400, "Role is required.")
    try:
        oid = ObjectId(user_id)
    except Exception:
        raise HTTPException(400, "Invalid user ID.")

    users_col = get_collection("users")
    user = users_col.find_one_and_update(
        {"_id": oid},
        {"$set": {"role": body.role}},
        return_document=True,
    )
    if not user:
        raise HTTPException(404, "User not found.")
    return {"user": _to_public_user(user)}
