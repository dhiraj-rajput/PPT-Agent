...
def _compute_match_score(notice_id: str) -> int:
    """Deterministic match score (70-98) seeded on noticeId — stable across restarts."""
...
def get_tenders(
    ...
    try:
        ...
        if naics and naics.upper() != "ALL":
            and_conditions.append({"naics_code": {"$regex": naics.strip(), "$options": "i"}})

        if set_aside and set_aside.upper() != "ALL":
            and_conditions.append({"$or": [
                {"set_aside_code": {"$regex": set_aside.strip(), "$options": "i"}},
                {"set_aside": {"$regex": set_aside.strip(), "$options": "i"}},
            ]})
...
@router.post("/sync")
def sync_tenders_from_sam(payload: dict = {}):
    """
...
def _upsert_tenders(opportunities: list, sync_params: dict) -> dict:
...
def _insert_mock_tenders(params: dict) -> dict:
    ...
    return _upsert_tenders(mock_raw, params)
...
def get_tender_detail(notice_id: str):
...
def request_draft(notice_id: str, payload: dict = {}):
    """
    ...
    record = {
        ...
        "requester": payload.get("requester", "John Doe"),
...
def get_draft_request_status(notice_id: str):
    """
