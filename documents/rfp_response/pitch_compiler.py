...
class PitchCompiler:
    ...
    def load_winner_profile(self, winner_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            escaped_winner = re.escape(winner_name)
            escaped_first = re.escape(first_word)
            profile = col.find_one({"$or": [
                {"company_name": {"$regex": f"^{escaped_winner}$", "$options": "i"}},
                {"company_name": {"$regex": escaped_winner, "$options": "i"}},
                {"company_name": {"$regex": f"^{escaped_first}\\b", "$options": "i"}},
