...
def _queue_pending_leads(campaign_id: ObjectId, daily_limit: int, base_time: Optional[datetime] = None):
...
def resume_campaign(id: str, current_user: dict = Depends(get_current_user)):
    ...
    queued = _queue_pending_leads(oid, daily_limit, base_time=schedule_start)
